...
# Since they do not line up exactly, find the eye or mouse point closest to the target and
# take the distance from that point.
# Average the distance across all points in the sequence and return that value.
def extractSeparation(mousepoints, eyepoints, direction):
    distance = 0
    counts = 0

    first_eye = first_nonzero(eyepoints)
    first_mse = first_nonzero(mousepoints)

    # if there are no non-zero entries for either eye or mouse return 0.
    if first_eye == -1 or first_mse == -1:
        return 0

    if first_eye >= first_mse:
        next_eye = first_eye
        while next_eye != -1 and next_eye < len(eyepoints):
            next_mse = find_closest(mousepoints, next_eye)
            if next_mse != -1:
                distance += eyepoints[next_eye].euclideanDistance(mousepoints[next_mse])
                counts += 1
            next_idx = first_nonzero(eyepoints[next_eye + 1:])
            next_eye = -1 if next_idx == -1 else next_idx + (next_eye + 1)

    else:
        next_mse = first_mse
        while next_mse != -1 and next_mse < len(mousepoints):
            next_eye = find_closest(eyepoints, next_mse)
            if next_eye != -1:
                distance += eyepoints[next_eye].euclideanDistance(mousepoints[next_mse])
                counts += 1
            next_idx = first_nonzero(mousepoints[next_mse + 1:])
            next_mse = -1 if next_idx == -1 else next_idx + (next_mse + 1)
...
def main(num_points):
    ...
    outputPath = os.path.join(os.getcwd(), "Users_time_aligned")
